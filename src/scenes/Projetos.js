...
class projetos extends Component {
  ...
  componentWillMount() {
    ...
    return fetch(url + 'projects/all' + '?page=' + this.state.page)
  ...
  pesquisar() {
    ...
    return fetch(url + 'projects/search/'+ this.state.op_pesquisa + '/' + this.state.string_pesquisa + '?page=' + this.state.page)
      .then((response) => response.json())
      .then((responseJson) => {
        ...
        <View style={{ flex: 1, backgroundColor: 'white' }}>
          ...
          <ScrollView style={{ flex: 1 }}>
              ...
              <View style={{ flexDirection:'row', width:'100%' }}>
                  ...
                  <Text style={{fontSize: 15, color: '#1E90FF', padding:10, marginTop:5}}>Pesquisar por:</Text>

                  <Picker
                    selectedValue={this.state.op_pesquisa}
                    style={{ height: 50, width: '67%' }}
                    onValueChange={(itemValue, itemIndex) => this.setState({op_pesquisa: itemValue})}>
                    <Picker.Item label="Título" value="titulo" />
                    <Picker.Item label="Aluno" value="aluno" />
                    <Picker.Item label="Tags de projeto" value="tags" />
                    <Picker.Item label="Descrição" value="descricao" />
                    <Picker.Item label="Cursos" value="curso" />
                  </Picker>
            ...
            <FlatList
              ...
              renderItem={({ item }) => {
                ...
                // console.log(urlimg2 + item.image);
                // console.log("opa");
